...
class DottyAndSuite extends AnyFreeSpec with Matchers {
  ...
  val path = Paths.get("src","test","scala","assets","dotty","and","and.txt")
  ...
  "Dotty Reference to Intersectiontypes can be processed to a plantUML png" in {
    ...
    val globalScope = scalameta.util.namespaces.collector.SourcesCollector(List(input))
    val umlCollector = UMLCollector(input,GlobalContext(globalScope.resultingMap))

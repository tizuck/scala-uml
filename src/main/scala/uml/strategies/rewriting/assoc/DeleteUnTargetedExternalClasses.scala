...
import uml.{ClassRef, ConcreteClass, RelationshipElement, UMLUnit}
...
object DeleteUnTargetedExternalClasses extends RewriteStrategy[(List[RelationshipElement],List[RelationshipElement])]{
    ...
    val f:Any => Any = u => u match {
      case u:UMLUnit =>
        u.copy(toplevelElements = u.toplevelElements.filterNot{
          case c:uml.externalReferences.ClassDefRef =>
  ...
  private def checkMatchingClassRef(difference: List[RelationshipElement],c:uml.externalReferences.ClassDefRef): Boolean = {
